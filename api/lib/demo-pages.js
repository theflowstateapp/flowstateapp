...
const { fetchDemoDataSafe, IST } = require('./demo-safe.js');
...
const DemoLayout = ({ children, title, description, meta, variant = "1", mockMode = false }) => {
    ...
    <header role="banner">
        <div class="header">
            <div class="container">
                ...
                ${mockMode ? '<div style="position: absolute; top: 20px; right: 20px; background: rgba(255,255,255,0.2); padding: 8px 12px; border-radius: 6px; font-size: 0.9rem;">Demo Safe Mode (mock data)</div>' : ''}
...
// Demo Page Renderers
async function renderDemoOverviewHTML(meta, variant = "1") {
  try {
    const demo = await fetchDemoDataSafe();
    const mockMode = demo.source === "mock" || demo.source === "empty";
    
    // Use demo data or fallback to empty arrays
    const workspace = demo.workspace || { id: "demo-mock", name: "FlowState Demo", timezone: IST };
    const tasks = demo.tasks || [];
    
    // Get metrics (simplified for mock mode)
    const completedTasks = mockMode ? tasks.filter(t => t.status === "COMPLETED") : await getCompletedTasksForISTWeek(workspace.id);
    const carryOverTasks = mockMode ? tasks.filter(t => t.status === "NEXT") : await getCarryOverTasksForISTWeek(workspace.id);
    const scheduledTotals = mockMode ? { totalHours: 2, totalMinutes: 30 } : await getScheduledTotalsForWeek(workspace.id);
    
    // Get next suggested task
    const nextSuggested = mockMode ? tasks.find(t => t.status === "NEXT") : await getNextSuggestedWithProposal(workspace.id);
...
async function renderDemoTasksHTML(meta, variant = "1") {
  try {
    const demo = await fetchDemoDataSafe();
    const mockMode = demo.source === "mock" || demo.source === "empty";
...
async function renderDemoHabitsHTML(meta, variant = "1") {
  try {
    const demo = await fetchDemoDataSafe();
    const mockMode = demo.source === "mock" || demo.source === "empty";
    
    const workspace = demo.workspace || { id: "demo-mock", name: "FlowState Demo", timezone: IST };
    const habitLogs = demo.habitLogs || [];
    const habits = demo.habits || [];
...
async function renderDemoJournalHTML(meta, variant = "1") {
  try {
    const demo = await fetchDemoDataSafe();
    const mockMode = demo.source === "mock" || demo.source === "empty";
    
    const workspace = demo.workspace || { id: "demo-mock", name: "FlowState Demo", timezone: IST };
    const journal = demo.journal || [];
...
async function renderDemoReviewHTML(meta, variant = "1") {
  try {
    const demo = await fetchDemoDataSafe();
    const mockMode = demo.source === "mock" || demo.source === "empty";
...
async function renderDemoAgendaHTML(meta, variant = "1") {
  try {
    const demo = await fetchDemoDataSafe();
    const mockMode = demo.source === "mock" || demo.source === "empty";
...
async function renderDemoSettingsHTML(meta, variant = "1") {
  try {
    const demo = await fetchDemoDataSafe();
    const mockMode = demo.source === "mock" || demo.source === "empty";
