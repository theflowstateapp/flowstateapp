// GET /api/brief/today
// Returns today's morning brief with planned tasks and proposals
...
module.exports = async (req, res) => {
  try {
    ...
    // Get morning window (09:30-12:30 IST)
    const morningWindow = getMorningWindow(nowIST);
    ...
    if (dayPlan && dayPlan.task_ids && dayPlan.task_ids.length > 0) {
      ...
      // Generate proposals for planned tasks
      items = (plannedTasks || []).map(task => {
        const proposal = proposeTimeBlocks(task, morningWindow);
        return {
          taskId: task.id,
          title: task.name,
          priority: task.priority_matrix || 'Priority 4. Low',
          estimateMins: task.estimated_hours ? task.estimated_hours * 60 : 30,
          context: null, // Not available in current schema
      ...
      // Generate proposals for candidate tasks
    ...
    return res.status(200).json({
      today,
      window: {
        start: morningWindow.start.toISOString(),
        end: morningWindow.end.toISOString()
      },
      items,
      strategy
    });
