    ...
    <header role="banner">
        <div class="header">
            <div class="container">
                ...
                ${mockMode ? '<div data-demo-mode="mock" style="position:absolute;right:12px;top:8px;font-size:12px;opacity:.7;">Demo Safe Mode (mock data)</div>' : ''}
...
// Demo Page Renderers
async function renderDemoOverviewHTML(meta, opts = {}) {
  try {
    const demo = await fetchDemoDataSafe({ force: opts.force });
...
async function renderDemoTasksHTML(meta, opts = {}) {
  try {
    const demo = await fetchDemoDataSafe({ force: opts.force });
    const mockMode = demo.source === "mock" || demo.source === "empty";
...
async function renderDemoHabitsHTML(meta, opts = {}) {
  try {
    const demo = await fetchDemoDataSafe({ force: opts.force });
    const mockMode = demo.source === "mock" || demo.source === "empty";
...
async function renderDemoJournalHTML(meta, opts = {}) {
  try {
    const demo = await fetchDemoDataSafe({ force: opts.force });
    const mockMode = demo.source === "mock" || demo.source === "empty";
...
async function renderDemoReviewHTML(meta, opts = {}) {
  try {
    const demo = await fetchDemoDataSafe({ force: opts.force });
    const mockMode = demo.source === "mock" || demo.source === "empty";
...
async function renderDemoAgendaHTML(meta, opts = {}) {
  try {
    const demo = await fetchDemoDataSafe({ force: opts.force });
    const mockMode = demo.source === "mock" || demo.source === "empty";
...
async function renderDemoSettingsHTML(meta, opts = {}) {
  try {
    const demo = await fetchDemoDataSafe({ force: opts.force });
    const mockMode = demo.source === "mock" || demo.source === "empty";
